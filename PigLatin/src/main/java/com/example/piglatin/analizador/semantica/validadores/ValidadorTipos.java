...
public class ValidadorTipos {
    ...
    private String inferirBinaria(NodoExpr.Binaria bin) {
        ...
        // Aritmeticos: jerarquia de tipos, resultado es el de mayor jerarquia
        if (op.equals("+") || op.equals("-") || op.equals("*") || op.equals("/")) {
            ...
            if (nivelIzq == 0 || nivelDer == 0) {
                reportarSiNoEsDesconocidoPropagado(bin.linea(), tipoIzq, tipoDer,
                        "Tipo no soportado en operacion aritmetica: " + tipoIzq + " " + op + " " + tipoDer);
                return "desconocido";
            }
        ...
        if (op.equals("<") || op.equals(">") || op.equals("<=") || op.equals(">=")
            ...
            if (nivelIzq == 0 || nivelDer == 0) {
                reportarSiNoEsDesconocidoPropagado(bin.linea(), tipoIzq, tipoDer,
                        "Tipo no soportado en comparacion: " + tipoIzq + " " + op + " " + tipoDer);
            }
            return "booleano";
        }

        // Logicos (&& ||): ambos operandos deben ser estrictamente booleano
        // ejemplo "verum || 1 == 1" combina booleanos, y la
        // seccion de condicionales exige evaluacion "estrictamente booleana").
        if (op.equals("&&") || op.equals("||")) {
            if (!"booleano".equals(tipoIzq) || !"booleano".equals(tipoDer)) {
                reportarSiNoEsDesconocidoPropagado(bin.linea(), tipoIzq, tipoDer,
                        "Operador logico " + op + " requiere operandos booleano, pero son: "
                                + tipoIzq + " y " + tipoDer);
            }
            return "booleano";
        }
    ...
    private String inferirAccesoArray(NodoExpr.AccesoArray acceso) {
        ...
        if (!"numerus".equals(tipoIndice) && !"desconocido".equals(tipoIndice)) {
            errores.add(new ErrorSemantico(acceso.linea(),
                    "Indice de arreglo debe ser numerus, pero es: " + tipoIndice));
        }
    ...
    private String inferirLlamadaFuncion(NodoExpr.LlamadaFuncion llamada) {
        ...
        if (opt.isEmpty()) {
            errores.add(new ErrorSemantico(llamada.linea(),
                    "Funcion no declarada: " + llamada.nombre()));
            return "desconocido";
        }
        ...
        if (params.size() != args.size()) {
            errores.add(new ErrorSemantico(llamada.linea(),
                    "Numero incorrecto de argumentos para " + llamada.nombre() +
                            ". Esperaba " + params.size() + ", recibio " + args.size()));
        }

        for (int i = 0; i < Math.min(params.size(), args.size()); i++) {
            String tipoArg = inferirTipo(args.get(i));
            String tipoParam = params.get(i);
            if (!"desconocido".equals(tipoArg) && !sonCompatibles(tipoArg, tipoParam)) {
