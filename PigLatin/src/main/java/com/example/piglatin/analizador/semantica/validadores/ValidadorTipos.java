/* version anterior antes de bool
...
public class ValidadorTipos {
    ...
    private String inferirBinaria(NodoExpr.Binaria bin) {
        ...
        // textum solo se concatena con +; nunca participa en comparaciones
        // ni en operadores lógicos (documentación: "El único tipo de dato
        // incompatible con todas las operaciones es el textum, el cual solo
        // se puede concatenar con los otros (con una suma)").
        if ("textum".equals(tipoIzq) || "textum".equals(tipoDer)) {
            if (!"+".equals(op)) {
                errores.add(new ErrorSemantico(bin.linea(),
                        "Textum solo se puede concatenar con +"));
        ...
        // Aritméticos: jerarquía de tipos, resultado es el de mayor jerarquía
        if (op.equals("+") || op.equals("-") || op.equals("*") || op.equals("/")) {
            int nivelIzq = JERARQUIA.getOrDefault(tipoIzq, 0);
            int nivelDer = JERARQUIA.getOrDefault(tipoDer, 0);
            if (nivelIzq == 0 || nivelDer == 0) {
                reportarSiNoEsDesconocidoPropagado(bin.linea(), tipoIzq, tipoDer,
                        "Tipo no soportado en operación aritmética: " + tipoIzq + " " + op + " " + tipoDer);
                return "desconocido";
            }
            return nivelIzq >= nivelDer ? tipoIzq : tipoDer;
        ...
        // Relacionales e igualdad: misma compatibilidad por jerarquía que
        // aritmética (documentación agrupa relacionales junto a aritméticos
        // bajo la misma regla de inferencia por jerarquía), pero el
        // resultado siempre es booleano, sin importar los tipos comparados.
        if (op.equals("<") || op.equals(">") || op.equals("<=") || op.equals(">=")
                || op.equals("==") || op.equals("!=")) {
            int nivelIzq = JERARQUIA.getOrDefault(tipoIzq, 0);
            int nivelDer = JERARQUIA.getOrDefault(tipoDer, 0);
            if (nivelIzq == 0 || nivelDer == 0) {
                reportarSiNoEsDesconocidoPropagado(bin.linea(), tipoIzq, tipoDer,
                        "Tipo no soportado en comparación: " + tipoIzq + " " + op + " " + tipoDer);
            }
            return "booleano";
        }

        // Lógicos (&& ||): ambos operandos deben ser estrictamente booleano
        // (documentación: ejemplo "verum || 1 == 1" combina booleanos, y la
        // sección de condicionales exige evaluación "estrictamente booleana").
        if (op.equals("&&") || op.equals("||")) {
            if (!"booleano".equals(tipoIzq) || !"booleano".equals(tipoDer)) {
                reportarSiNoEsDesconocidoPropagado(bin.linea(), tipoIzq, tipoDer,
                        "Operador lógico " + op + " requiere operandos booleano, pero son: "
                                + tipoIzq + " y " + tipoDer);
            }
            return "booleano";
        }

        // No debería alcanzarse con el conjunto de operadores del lenguaje.
        return "booleano";
    }

    // Evita reportar un segundo error cuando el tipo ya es "desconocido" por
    // una causa distinta ya reportada en otro lado (ej. variable no
    // declarada, ya capturado por ValidadorAlcance) — un solo error por
    // causa raíz, en vez de un error en cascada por cada uso de esa expresión.
    private void reportarSiNoEsDesconocidoPropagado(int linea, String tipoIzq, String tipoDer, String mensaje) {
        if ("desconocido".equals(tipoIzq) || "desconocido".equals(tipoDer)) {
            return;
        }
